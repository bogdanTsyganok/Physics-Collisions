...
float cParticleContact::CalculateSeparatingVelocity() const
{
	...
	return glm::dot(relativeVelocity, mContactNormal);//negative if it's not already seperating
}
...
void cParticleContact::ResolveVelocity(float deltaTime)
{
	...
	if (separatingVelocity > 0.0f)//check that it's negative (needs to be seperated)
	{
...
void cParticleContact::ResolvePosition(float deltaTime)
{
	...
	{
		mMovement0 = glm::vec3(0.0f, 0.0f, 0.0f);
		mMovement1 = glm::vec3(0.0f, 0.0f, 0.0f);
		return;
	}
